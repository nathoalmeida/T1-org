...
## Armazena o valor de buffer_leitura para memoria_text e incrementa o endereço de memoria_text
	lw $a2, endereco_data		# carrega o endereço inicial de memoria_texto em $a2
	...
	la $s0, endereco_data 		# carrega o endereço da variável endereco_texto
	addi $t1, $a2, 4		# incrementa 4 bytes no endereço de memoria
	sw $t1, 0($s0)			# armazena o novo valor do endereco_texto
## DO-WHILE:
	bgtz $v0, leituraArquivoData 	# se o número de caracteres lidos for maior que 0, continua a leitura do arquivo
	jr	$ra
...
instrucaoJ:
	sll $t0, $s5, 21  			## desloca campo rs 21 bits para esquerda
...
instJ:
	lw	$t0, 0($s1)			# $t0 -> address (26 bits) 
    	...
    	lw	$t2, endereco_texto		# $t2 -> endereço inicial da memória de texto
    	sll	$t0, $t0, 2			# $t0 -> address * 4
    	add 	$t0, $t0, $t2			# $t0 <- endereço efetivo da label desejada
    	sw	$t0, 0($t1)			# Armazena em PC o endereço da instrução definida pela label
...
instJAL:
	...
	sw $t1, 31($t0)				 ## armazena endereço da próxima instrução em $ra 
...
instSYS:
	...
	sw $t1, ($v0)				 ## Armazena o valor em $v0
	syscall
...
.data
	...
	local_arquivo_data: 	.asciiz "trabalh0_01-2024_1.dat"
